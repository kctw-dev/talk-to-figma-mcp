...
server.tool(
  ...
  {
    nodeId: z.string().describe("The ID of the node to set the image fill on"),
    imageUrl: z.string().describe("The URL of the image to use as fill"),
    scaleMode: z.enum(["FILL", "FIT", "CROP", "TILE"]).optional().default("FILL").describe("How to scale the image within the node"),
  },
  async ({ nodeId, imageUrl, scaleMode }: any) => {
    try {
      ...
      const typedResult = result as { nodeId: string; imageHash: string };
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  {
    nodeId: z.string().describe("The ID of the node to set effects on"),
    effects: z.array(z.object({
      type: z.enum(["DROP_SHADOW", "INNER_SHADOW", "LAYER_BLUR", "BACKGROUND_BLUR"]).describe("The type of effect"),
  ...
  async ({ nodeId, effects }: any) => {
    try {
      ...
      const typedResult = result as { nodeId: string; effectCount: number };
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  "Set font family, style, and size on a text node",
  {
    ...
    fontStyle: z.string().optional().default("Regular").describe("Font style (e.g., Regular, Bold, Italic)"),
    fontSize: z.number().optional().describe("Font size in pixels"),
  },
  async ({ nodeId, fontFamily, fontStyle, fontSize }: any) => {
    try {
      ...
      const typedResult = result as { nodeId: string; fontFamily: string; fontStyle: string };
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  "Create a new page in the Figma document",
  {
    ...
    setCurrent: z.boolean().optional().describe("Whether to switch to the new page"),
  },
  async ({ name, setCurrent }: any) => {
    try {
      ...
      const typedResult = result as { pageId: string; pageName: string };
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  {
    pageId: z.string().optional().describe("The ID of the page to switch to"),
    pageName: z.string().optional().describe("The name of the page to switch to"),
  },
...
server.tool(
  ...
  {
    nodeId: z.string().describe("The ID of the node to set the gradient on"),
    stops: z.array(z.object({
      ...
      position: z.number().describe("Position of this stop (0-1)"),
    })).describe("Array of gradient color stops"),
    gradientType: z.enum(["GRADIENT_LINEAR", "GRADIENT_RADIAL", "GRADIENT_ANGULAR", "GRADIENT_DIAMOND"]).optional().default("GRADIENT_LINEAR").describe("Type of gradient"),
    angle: z.number().optional().describe("Angle of the gradient in degrees (for linear gradients)"),
  },
  async ({ nodeId, stops, gradientType, angle }: any) => {
    try {
      ...
      const typedResult = result as { nodeId: string; stopCount: number };
      return {
        content: [
          {
            type: "text",
...
// Scan Instance Keys — get component keys from all instances under a node
server.tool(
  "scan_instance_keys",
...
// Get Instance Info — get mainComponent key from a single instance
server.tool(
  "get_instance_info",
...
// Set Instance Properties — switch variant, toggle boolean props, etc.
server.tool(
...
// Detach Instance — convert library instance to editable frame
server.tool(
  "detach_instance",
...
// Walk and Replace Text — brute force text replacement by walking children tree
server.tool(
...
// Walk and Scan Text — brute force text scan by walking children tree
server.tool(
  "walk_and_scan_text",
...
type CommandParams = {
  ...
  import_component_by_key: { key: string; x?: number; y?: number; parentId?: string };
  import_component_set_by_key: { key: string };
  import_style_by_key: { key: string };
  scan_instance_keys: { nodeId?: string };
  get_instance_info: { nodeId: string };
  set_instance_properties: { nodeId: string; properties: Record<string, any> };
  detach_instance: { nodeId: string };
  walk_and_replace_text: { nodeId: string; replacements: Array<{ from: string; to: string; contains?: boolean }> };
  walk_and_scan_text: { nodeId: string };
};
