  ...
  ws.on("message", (data) => {
    try {
      ...
      if (json.type === "auth_challenge" && json.nonce) {
        ...
        const signature = cryptoSign("SHA256", Buffer.from(json.nonce), {
...
server.tool(
  ...
  "Create a Figma variable collection with variables. Supports COLOR, FLOAT, STRING, and BOOLEAN types with multiple modes (e.g., light/dark theme).",
  {
    ...
    modes: z.array(z.string()).optional().describe(
      "Mode names (e.g., ['Light', 'Dark']). First mode replaces the default mode."
    ),
    variables: z.array(
      z.object({
        name: z.string().describe("Variable name (e.g., 'primary-color')"),
        resolvedType: z.enum(["COLOR", "FLOAT", "STRING", "BOOLEAN"]).default("COLOR").describe("Variable data type"),
        values: z.array(z.any()).optional().describe(
