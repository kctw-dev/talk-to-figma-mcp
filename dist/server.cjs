...
server.tool(
  ...
  "Bind a Figma variable to a node's fill color",
  {
    ...
    variableName: import_zod.z.string().describe("The name of the Figma variable to bind")
  },
...
server.tool(
  ...
  "Create an instance of a component in Figma",
  {
    ...
    componentId: import_zod.z.string().optional().describe("Node ID of a local component to instantiate"),
    parentId: import_zod.z.string().optional().describe("Optional parent node ID to append the instance to"),
    x: import_zod.z.number().describe("X position"),
    y: import_zod.z.number().describe("Y position")
  },
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("Node to move"),
    parentId: import_zod.z.string().optional().describe("New parent (defaults to current page)"),
    index: import_zod.z.number().optional().describe("Insert position among the parent's children"),
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).describe("COMPONENT node ids to combine"),
    parentId: import_zod.z.string().optional().describe("Where to put the resulting set (defaults to current page)"),
...
server.tool(
  ...
  "Create nodes from SVG source (figma.createNodeFromSvg). The only way to author real path/vector geometry \u2014 use it for icons the design system is missing. create_tree also accepts {type:'SVG', svg:'<svg\u2026>'} spec nodes.",
  {
    items: import_zod.z.array(import_zod.z.object({
      svg: import_zod.z.string().describe("Full <svg>\u2026</svg> source"),
...
server.tool(
  ...
  {
    items: import_zod.z.array(import_zod.z.object({
      nodeId: import_zod.z.string().describe("INSTANCE node id (nested ids like 'I123:4;56:7' work)"),
      componentId: import_zod.z.string().describe("COMPONENT (or COMPONENT_SET \u2014 uses its default variant) to swap in")
...
server.tool(
  ...
  {
    parentId: import_zod.z.string().optional().describe("Parent node to append into (defaults to current page)"),
    spec: import_zod.z.any().optional().describe("A single root spec object"),
    specs: import_zod.z.array(import_zod.z.any()).optional().describe("Several root specs appended in order")
  },
...
server.tool(
  ...
  {
    bindings: import_zod.z.array(import_zod.z.object({
      nodeId: import_zod.z.string(),
      variableName: import_zod.z.string(),
...
server.tool(
  ...
  {
    items: import_zod.z.array(import_zod.z.any()).describe("List of {nodeId, ...props} updates")
  },
  async ({ items }) => {
    try {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("Node to read"),
    maxDepth: import_zod.z.number().optional().describe("How deep to walk (default 3)")
  },
...
server.tool(
  ...
  {
    query: import_zod.z.string().describe("Case-insensitive substring to match against component names"),
    limit: import_zod.z.number().optional().describe("Max results (default 50)")
  },
  async ({ query, limit }) => {
    try {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to bind the variable to"),
    variableName: import_zod.z.string().describe("Exact full name of the local variable (from get_local_variables)"),
    field: import_zod.z.string().describe("The bindable property name, e.g. cornerRadius, itemSpacing, paddingLeft, fontSize")
  },
...
server.tool(
  ...
  {
    collectionName: import_zod.z.string().optional().describe("Only return the collection with this exact name"),
    summaryOnly: import_zod.z.boolean().optional().describe("Return only modes and the alias/raw tally, omitting the full variable list")
  },
...
server.tool(
  ...
  {
    collectionKey: import_zod.z.string().describe("The key or name of the library collection (from get_library_collections)")
  },
  async ({ collectionKey }) => {
    try {
...
server.tool(
  ...
  {
    key: import_zod.z.string().describe("The component key (from get_library_components)"),
...
server.tool(
  ...
  {
    key: import_zod.z.string().describe("The component set key")
  },
  async ({ key }) => {
    try {
...
server.tool(
  ...
  {
    key: import_zod.z.string().describe("The style key")
  },
  async ({ key }) => {
    try {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().optional().describe("The node ID to scan (defaults to current page)")
  },
  async ({ nodeId }) => {
    try {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The instance node ID")
  },
  async ({ nodeId }) => {
    try {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The instance node ID"),
    properties: import_zod.z.record(import_zod.z.any()).describe('Properties to set, e.g. {"Platform": "Mobile"}')
  },
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The instance node ID to detach")
  },
  async ({ nodeId }) => {
    try {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("Root node ID to start walking from"),
    replacements: import_zod.z.array(import_zod.z.object({
      from: import_zod.z.string().describe("Exact text to match"),
      to: import_zod.z.string().describe("Replacement text"),
      contains: import_zod.z.boolean().optional().describe("If true, match partial text")
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to set reactions on"),
    reactions: import_zod.z.array(import_zod.z.object({
      trigger: import_zod.z.object({
        type: import_zod.z.enum(["ON_CLICK", "ON_HOVER", "ON_PRESS", "ON_DRAG", "MOUSE_ENTER", "MOUSE_LEAVE", "MOUSE_UP", "MOUSE_DOWN", "AFTER_TIMEOUT"]).default("ON_CLICK").describe("Trigger type"),
        timeout: import_zod.z.number().optional().describe("Timeout in seconds (for AFTER_TIMEOUT trigger)")
      }).optional().describe("Trigger for this reaction (defaults to ON_CLICK)"),
      destinationId: import_zod.z.string().optional().describe("Shorthand: destination node ID for simple navigation"),
      navigation: import_zod.z.enum(["NAVIGATE", "OVERLAY", "SWAP", "SCROLL_TO", "CHANGE_TO"]).optional().describe("Navigation type (default: NAVIGATE)"),
      transition: import_zod.z.object({
        type: import_zod.z.enum(["DISSOLVE", "SMART_ANIMATE", "MOVE_IN", "MOVE_OUT", "PUSH", "SLIDE_IN", "SLIDE_OUT"]).default("DISSOLVE"),
        easing: import_zod.z.object({
          type: import_zod.z.enum(["EASE_IN", "EASE_OUT", "EASE_IN_AND_OUT", "LINEAR", "EASE_IN_BACK", "EASE_OUT_BACK", "EASE_IN_AND_OUT_BACK"]).default("EASE_IN_AND_OUT")
        }).optional(),
        duration: import_zod.z.number().default(0.3).describe("Duration in seconds")
      }).optional().describe("Transition animation"),
      actions: import_zod.z.array(import_zod.z.object({
        type: import_zod.z.enum(["NODE", "BACK", "CLOSE", "URL"]).describe("Action type"),
        destinationId: import_zod.z.string().optional().describe("Target node ID (for NODE action)"),
        navigation: import_zod.z.string().optional().describe("Navigation type"),
        transition: import_zod.z.any().optional().describe("Transition config"),
        url: import_zod.z.string().optional().describe("URL (for URL action)")
...
server.tool(
  ...
  {
    collectionName: import_zod.z.string().describe("Name for the variable collection"),
    modes: import_zod.z.array(import_zod.z.string()).optional().describe(
      "Mode names (e.g., ['Light', 'Dark']). First mode replaces the default mode."
    ),
    variables: import_zod.z.array(
      import_zod.z.object({
        name: import_zod.z.string().describe("Variable name (e.g., 'primary-color')"),
        resolvedType: import_zod.z.enum(["COLOR", "FLOAT", "STRING", "BOOLEAN"]).default("COLOR").describe("Variable data type"),
        values: import_zod.z.array(import_zod.z.any()).optional().describe(
  ...
  async ({ collectionName, modes, variables }) => {
    try {
